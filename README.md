# ToDo Application with a Serverless API and Azure Cosmos DB API for MongoDB on Static Web Apps
...
A complete ToDo application that includes everything you need to build, deploy, and monitor an Azure solution. This application uses the Azure Developer CLI (azd) to get you up and running on Azure quickly, React.js for the Web application, Azure Functions (Node.js) for the API, Azure Cosmos DB API for MongoDB for storage, and Azure Monitor for monitoring and logging. It includes application code, tools, and pipelines that serve as a foundation from which you can build upon and customize when creating your own solutions. For local development, the Azure Static Web Apps CLI (swa) is included to run and emulate the application locally.
...
```bash
azd init --template manekinekko/todo-nodejs-mongo-swa
```
...
- ToDo Web application frontend (with its API backend)
...
- [**Azure Key Vault**](https://docs.microsoft.com/azure/key-vault/) for securing secrets (optional)
...
To debug applications built for Azure Static Web Apps, you can refer to the [SWA CLI documentation](https://azure.github.io/static-web-apps-cli/docs/use/emulator), or the [VS Code extension documentation](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurestaticwebapps).
